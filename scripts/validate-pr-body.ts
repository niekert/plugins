 ...
 * Used by the Shippy workflow to check PRs with "Submit Plugin" label.
 *
 * Usage: yarn tsx scripts/validate-pr-body.ts
 *
 * Environment Variables:
 *   PR_BODY         - The PR body text to validate
 *   REQUIRE_CHANGELOG - Set to "true" to require changelog (when Submit Plugin label is present)
 *
 * Exit codes:
 *   0 - Validation passed
 *   1 - Validation failed
...
if (requireChangelog) {
    ...
    if (!changelog) {
        console.log(
            "❌ Changelog required when 'Submit Plugin' label is applied. Add content to the '### Changelog' section in your PR description."
        )
        process.exit(1)
    }
